defmodule Pointers.Schema do
  ...
  @doc """
  Defines a schema that can be pointed to - one keyed by autogenerated
  ULID that you promise to have run the corresponding migrations for.

  Takes a ULID in text form which is a sentinel value used to identify
  the table. It *must* match the one inserted into `Table` in the
  migrations that create this table.
  """
  ...
  @doc """
  Define a trait schema - one that keyed by ULID, is not pointable and
  does not autogenerate IDs. Takes a trait name (atom) which will be
  used to autogenerate alias names *sigh*
  """
  defmacro trait_schema(table, name, body) when is_atom(name) do
    quote do
      Pointers.Schema.ulid_schema(unquote(table), false, unquote(body))
    end
  end
